...
이 문서는 주문팡팡 주문관리 MVP에서 스마트스토어 주문을 실제 네이버 커머스API와 연동할 때 필요한 기준을 정리한다.
...
주문팡팡 MVP의 1차 연동 범위는 `인증`, `주문`, `판매자정보`다.

## 2.1 기능별 공식 문서 확인 위치

실제 구현 시에는 아래 문서를 먼저 확인한다.

| 주문팡팡 기능 | 먼저 확인할 공식 문서 |
|---|---|
...
| 주문팡팡 화면/기능 | 네이버 API 연동 필요성 | MVP 연동 기준 |
|---|---|---|
| 마켓연동 | 필요 | 스마트스토어 계정의 인증 정보 저장 및 토큰 발급 준비 |
| 주문수집 | 필요 | 변경 상품 주문 내역 조회 후 상세 조회 |
| 주문확인 | 필요 | 발주 확인 처리 |
| 매칭하기/소싱하기 | 직접 연동 없음 | 주문팡팡/소싱라이프 내부 흐름 |
| 발송대기 | 필요 | 국내송장 확보 후 발송 처리 |
| 배송중 | 조회 중심 | 변경된 송장번호가 있으면 내부 표시만 하고 네이버 송장 수정 전송은 MVP 제외 |
...
주의:
- 네이버 발주확인은 공식 상태 흐름상 상품주문상태를 별도 상태로 바꾸지 않을 수 있다. 주문팡팡은 내부 처리 단계 관리를 위해 발주확인 성공 후 `PREPARING`으로 이동한다.
- 네이버 발송처리는 배송중 전이를 일으키는 핵심 API다.
- 배송완료 이벤트는 별도 이벤트가 항상 제공되는 구조가 아닐 수 있으므로 변경분 조회와 상세 조회 결과를 함께 본다.

## 7. 주문확인 기준

주문팡팡의 `주문확인` 버튼은 네이버의 발주 확인 처리에 대응한다.
...
주문팡팡의 `배송중 처리` 버튼은 네이버의 발송 처리에 대응한다.
...
| `dispatchProductOrders.deliveryCompanyCode` | 택배사 코드 | 주문팡팡 택배사 라벨을 네이버 코드로 변환 |
...
- 배송중 이후 변경된 송장번호가 있으면 내부 화면에 참고용으로만 표시하고, 실제 마켓 수정은 판매자센터에서 처리하는 기준으로 둔다.
...
주문팡팡의 `주문취소`는 네이버의 판매자 직접 취소 요청 또는 고객 취소 요청 승인과 구분해서 설계한다.
...
- 택배사 코드는 발송 처리 전에 네이버 코드와 주문팡팡 택배사 라벨 간 매핑 테이블을 별도로 관리한다.
...
1. 네이버 응답 상태 enum과 주문팡팡 내부 `OrderStatus` 최종 매핑
